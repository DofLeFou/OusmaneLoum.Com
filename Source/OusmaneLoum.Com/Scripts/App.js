...
(function (ol) {
    var initialId;
    ol.initialId = initialId;
}(window.OusmaneLoum));

(function (ol) {
    var initialState;
    ol.initialState = initialState;
}(window.OusmaneLoum));

(function (ol) {
    var rootPath;
    ol.rootPath = rootPath;
}(window.OusmaneLoum));
...
(function (ol) {
    var viewModelHelper = function () {
        ...
        self.apiGet = function (uri, data, success, failure, always) {
            ...
            $.get(OusmaneLoum.rootPath + uri, data)
                .done(success)
        ...
        self.apiPost = function (uri, data, success, failure, always) {
            self.isLoading(true);
            self.modelIsValid(true);
            $.post(OusmaneLoum.rootPath + uri, data)
                .done(success)
                .fail(function (result) {
                    if (failure == null) {
                        if (result.status != 400)
        ...
        self.pushUrlState = function (code, title, id, url) {
            self.stateInfo = { State: { Code: code, Id: id }, Title: title, Url: OusmaneLoum.rootPath + url };
        }
    ...
    ol.viewModelHelper = viewModelHelper;
}(window.OusmaneLoum));
